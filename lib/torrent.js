...
class Torrent extends EventEmitter {
  ...
  _verifyPieces (cb) {
    if (this.store.store.verifyPieces)
      this.store.store.verifyPieces(this._hashes, (error, verifyResults) => {
        if (this.destroyed) return cb(new Error('torrent is destroyed'))
        if (error) return process.nextTick(cb, null)
        
        for (var index = 0; index < verifyResults.length; index ++) {
          if (verifyResults.charCodeAt(index) == 49) {   //49: "1"
